...
if #_menuPlugins > 0 then
    -- Wrap userMenu: intercept clickMenu call to inject plugin entries
    local _origUserMenu = userMenu
    userMenu = function()
        local _origClickMenu = clickMenu
        clickMenu = function(title, items, msg)
            -- Only inject into the main user menu call
            -- Detect it by checking if items contains "Logout" as last entry
            local isMainMenu = items and #items > 0 and
                items[#items].label == "Logout"
            if isMainMenu then
                -- inject plugin entries before Logout
                local newItems = {}
                for i = 1, #items - 1 do
                    table.insert(newItems, items[i])
                end
                for _, p in ipairs(_menuPlugins) do
                    table.insert(newItems, { label = p.label or p.name, icon = colors.purple })
                end
                table.insert(newItems, items[#items])  -- Logout

                -- override title to use nickname if set
                local displayTitle = title
                if _G.cloudDisplayName then
                    displayTitle = title:gsub(username, _G.cloudDisplayName, 1)
                end

                clickMenu = _origClickMenu
                local sel = _origClickMenu(displayTitle, newItems, msg)

                -- remap selection back: if it's a plugin slot, call it and return nil to loop
                if sel and sel > #items - 1 and sel < #newItems then
                    local idx = sel - (#items - 1)
                    if _menuPlugins[idx] then
                        _menuPlugins[idx].run()
                        clickMenu = function(t, it, m)
                            return userMenu_intercept(t, it, m)
                        end
                    end
                    return nil  -- loop userMenu again
                end

                return sel
            end
            clickMenu = _origClickMenu
            return _origClickMenu(title, items, msg)
        end
        _origUserMenu()
        clickMenu = _origClickMenu
    end
...
-- ── Write and run instance ────────────────────────────────────────────────────
-- Só cria a instância se ela não existir, evitando resetar os menus no loop
if not fs.exists(INSTANCE) then
